...
contract Neurinx is Ownable, IBEP20 {
	...
	mapping (address => bool) private _isNohumaned;

	string private constant _name = "Neurinx";
	string private constant _symbol = "NUIX";
	uint8 private constant _decimals = 8;

	uint256 private constant _totalSupply = 1000000000000000 * (10 ** _decimals);  // 100000000000000000000000
    uint256 public _maxWallet = 550000000000000 * (10 ** _decimals);               //  55000000000000000000000 55% MAX
	uint256 public swapForLiquidityThreshold = 10000000000000 * (10 ** _decimals); //   1000000000000000000000
	...
	address public constant LIQUIDITY = address(0x2706Fe1478e90573534c9Dd05C0fd83077aC8aD8);
	...
	function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
		...
		require(currentAllowance >= amount, "NUIX: transfer amount exceeds allowance");
	...
	function decreaseAllowance(address spender, uint256 subtractedValue) external returns (bool) {
		...
		require(currentAllowance >= subtractedValue, "NUIX: decreased allowance below zero");
	...
	function _approve(address _owner, address spender, uint256 amount) private {
		require(_owner != address(0), "NUIX: approve from the zero address");
		require(spender != address(0), "NUIX: approve to the zero address");
	...
	function _transfer(address sender, address recipient, uint256 amount) private {
		require(sender != address(0), "NUIX: transfer from the zero address");

		uint256 senderBalance = _balances[sender];
		require(senderBalance >= amount, "NUIX: transfer amount exceeds balance");

		require(!_isNohumaned[sender] && !_isNohumaned[recipient], "You are a not human");
	...
	function sellingTax(address sender, uint256 amount) private returns (uint256) {
		...
		if (newCumSum > totalSupply().mul(MAX_SELL).div(10**3)) {
			revert("NUIX: selling amount is above max allowed");
		}
	...
	function balancer(uint256 amount) private {
		...
		_balances[LIQUIDITY] += half;
		emit Transfer(_msgSender(), LIQUIDITY, half);
	...
	function updateIsNohumaned(address account, bool state) external onlyOwner {
		_isNohumaned[account] = state;
	}

	function bulkIsNohumaned(address[] memory accounts, bool state) external onlyOwner {
		for(uint256 i =0; i < accounts.length; i++){
			_isNohumaned[accounts[i]] = state;
